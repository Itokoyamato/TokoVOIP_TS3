	...
	Citizen.CreateThread(function()
		while (true) do
			...
			if (self.lastNetworkUpdate >= self.networkRefreshRate) then		--	Update the ped's network data
				DecorSetInt(PlayerPedId(), "voip:mode", self.mode);
				DecorSetInt(PlayerPedId(), "radio:channel", self.data.radioChannel);
				DecorSetInt(PlayerPedId(), "radio:talking", self.data.radioTalking);
				DecorSetInt(PlayerPedId(), "voip:pluginStatus", self.pluginStatus);
				DecorSetInt(PlayerPedId(), "voip:pluginVersion", self.pluginVersion);
				self.lastNetworkUpdate = 0;
		...
		while (true) do
			...
			if (IsControlPressed(0, Keys["LEFTSHIFT"])) then	--	Switch radio channels (main / shared)
				if (IsControlJustPressed(0, Keys["Z"]) and tablelength(self.myChannels) > 0) then
					...
					DecorSetInt(PlayerPedId(), "radio:channel", currentChannelID);
					self.updateTokoVoipInfo(self);
				end
			...
			if (IsControlPressed(0, Keys["CAPS"]) and self.data.radioChannel ~= 0) then	--	Talk on radio
				...
				DecorSetInt(PlayerPedId(), "radio:talking", true);
				self.updateTokoVoipInfo(self);
				...
				DecorSetInt(PlayerPedId(), "radio:talking", false);
				self.updateTokoVoipInfo(self);
